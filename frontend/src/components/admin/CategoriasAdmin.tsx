...
function CategoriasAdminContent() {
  ...
  const handleSubmit = async (data: Omit<Categoria, "id">) => {
    setError(null);
    try {
      if (editing === "new") {
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      await categoriasApi.remove(deleting.id);
      setDeleting(null);
      await load();
    } catch (err) {
    ...
    <div>
      ...
      {deleting && (
        <ConfirmDialog
          ...
          message={`¿Eliminar la categoría "${deleting.nombre}"?`}
